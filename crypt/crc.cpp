...
uint32_t crc32(uint8_t* data_in, uint16_t bytes){
	...
	for(uint32_t i = 0; i < bytes; i++){
		crc32 = (crc32 >> 8) ^ crc32_tab[(crc32 & 0xff) ^ data_in[i]];
	}
...
uint8_t crc8(uint8_t* data_in, uint16_t bytes){
  uint8_t  crc;
  uint16_t loop_count;
  uint8_t  bit_counter;
  uint8_t  data;
  ...
  for (loop_count = 0; loop_count != bytes; loop_count++)
  {
